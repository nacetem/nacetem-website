...
export default function UploadsCredentials() {
  ...
  const getStudents = () => {
    axios
      .get(`${API_URL}/preview/${student_id}`, {
  ...
  const handleOnSubmit = async (event) => {
    ...
    try {
      if (file) {
        ...
        setErrorMsg('');
        const result = await axios.put(`${API_URL}/uploads/${student_id}`, formData, {
          headers: {
 ...
 return <>
    ...
    <Grid item container direction="row" >
      ...
      autoHideDuration={4000} onClose={handleClose} key={vertical + horizontal}>
          ...
          <b> Saved successfully. Continueing to Personal Details</b>
          </Alert>
        </Snackbar>
        ...
        <form encType="multipart/form-data" className={mstyles.searchForm} onSubmit={handleOnSubmit}>
          ...
          <Grid item> 
          <img src={students} alt="photo preview"/>
